import schedulePayload from "@/data/skjolden-cruise-schedules.generated.json";

export type SkjoldenScheduleRow = {
  port: string;
  ship: string;
  cruise_line: string;
...
export type SkjoldenScheduleEntry = {
  date: string;
  ship: string;
  arrival: string;
  departure: string;
...
type SchedulePayload = {
  ...
  rows: SkjoldenScheduleRow[];
};

const payload = schedulePayload as SchedulePayload;

export const skjoldenScheduleIntegrity = payload.integrity;
export const skjoldenScheduleMeta = {
  port: payload.port,
...
function toEntry(row: SkjoldenScheduleRow): SkjoldenScheduleEntry {
  return {
...
export function getAllSkjoldenRows(): SkjoldenScheduleRow[] {
  return payload.rows;
}

export function getSkjoldenEntries(): SkjoldenScheduleEntry[] {
  return payload.rows.map(toEntry);
}

export function getSkjoldenEntriesForMonthKey(
  monthKey: string,
): SkjoldenScheduleEntry[] {
  return payload.rows
    .filter((row) => row.arrival_date.startsWith(monthKey))
    .map(toEntry)
    .sort(
      (a, b) => a.date.localeCompare(b.date) || a.ship.localeCompare(b.ship),
    );
}

export function getSkjoldenMonthKeysWithCalls(): string[] {
  return [
    ...new Set(payload.rows.map((row) => row.arrival_date.slice(0, 7))),
  ].sort();
}

export function getSkjoldenMonthSummaries() {
